...
namespace TGS.Challenge
{
    ...
    public class FormatNumber
    {
        public string Format(int value)
        {
            ...
            var numarray = value.ToString().ToCharArray();
            int lennum = numarray.Length;
            Array.Reverse(numarray);
            List<char> newnumarray = new List<char>();
            int counter = 0;
            for(int i=0;i<lennum;i++)
            {
                newnumarray.Add(numarray[i]);
                counter++;
                if(counter==3 && i !=lennum-1)
                {
                    newnumarray.Add(',');
                    counter = 0;
            ...
            string FormattedNum = new string(newnumarray.ToArray());
